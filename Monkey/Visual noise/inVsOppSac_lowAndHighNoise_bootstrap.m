...
jumpSize = abs(cleanData.jumpSize(valid));
...
lowPriorNoNoiseResp = resp(lowPrior & noNoise);
highPriorNoNoiseResp = resp(highPrior & noNoise);
...
% cd('C:\Users\Divya\Documents\Divya data\Non-Bayesian paper\Skwiz-prior+noise-behaviorOnly\Paper summary data')
cd('C:\Users\Divya\Documents\Divya data\Non Bayesian behavior paper data\Monkey\Monkey S Visual Exp\inVsOppSac')
save('inSac_noNoise_Bootstrap.mat', 'binSize', 'baseline', 'baselineAll', 'lowPriorNoNoise', 'lowPriorNoNoiseAll', 'highPriorNoNoise', 'highPriorNoNoiseAll', 'L', 'U', 'st')

%low noise
%low prior
lowPriorLowNoiseJS = abs(jumpSize(lowPrior & lowNoise));
lowPriorLowNoiseResp = resp(lowPrior & lowNoise);
...
highPriorLowNoiseResp = resp(highPrior & lowNoise);
...
lowPriorHighNoiseResp = resp(lowPrior & highNoise);
...
highPriorHighNoiseResp = resp(highPrior & highNoise);
...
% cd('C:\Users\Divya\Documents\Divya data\Non-Bayesian paper\Skwiz-prior+noise-behaviorOnly\Paper summary data')
cd('C:\Users\Divya\Documents\Divya data\Non Bayesian behavior paper data\Monkey\Monkey S Visual Exp\inVsOppSac')
save('inSac_lowAndHighNoise_bootstrap.mat', 'jumpSizes', 'lowPriorLowNoiseMean', 'lowPriorLowNoiseCI', 'lowPriorLowNoiseAll', 'lowPriorLowNoiseZeroResp', 'highPriorLowNoiseMean', 'highPriorLowNoiseCI', 'highPriorLowNoiseAll', 'highPriorLowNoiseZeroResp', 'lowPriorHighNoiseMean', 'lowPriorHighNoiseCI', 'lowPriorHighNoiseAll', 'lowPriorHighNoiseZeroResp', 'highPriorHighNoiseMean', 'highPriorHighNoiseCI', 'highPriorHighNoiseAll', 'highPriorHighNoiseZeroResp')
