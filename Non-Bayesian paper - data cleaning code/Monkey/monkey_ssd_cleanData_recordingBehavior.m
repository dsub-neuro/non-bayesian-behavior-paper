function cleanData = monkey_ssd_cleanData_recordingBehavior(sessionData, trialData)
...
for trialNum = 1:nTrials
    ...
    if length(photoEvents) > 2 %if reafferent target came on
        ...
        %screen
        fixLoc(trialNum, 1) = sessionData.fixLocStore(trialNum, 1); %X position in degrees 
        fixLoc(trialNum, 2) = sessionData.fixLocStore(trialNum, 2); %Y position in degrees 
        ...
        elseif abs(targLoc(trialNum, 1) - fixLoc(trialNum, 1)) > abs(targLoc(trialNum, 2) - fixLoc(trialNum, 2)) %more horizontal
        ...
        if sacDir(trialNum) == 0
            ...
            sacVelStart = find(abs(sacVel) > 0.2, 1, 'first');
            if ~isempty(sacVelStart) && sacVelStart > 20 && sacVelStart < 500 %if true saccade and not spurious exiting of window after time limit
                sacEnd = find(abs(sacVel(sacVelStart:end)) < 0.01, 1, 'first'); %find saccade endpoint from start of saccade to end
...
cleanData.validTrials = find(cleanTrials == 1);
cleanData.probeFixDur = sessionData.probeFixDurStore(cleanTrials);
cleanData.fixDur = sessionData.fixDurStore(cleanTrials);
end