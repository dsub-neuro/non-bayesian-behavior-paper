...
for participant = 1:n
    ...
    [min(participant, 5), slope(participant, 5), threshold(participant, 5), max(participant, 5), lowPriorLowNoisePsychCurves(participant, :), ~] = psychCurve_4P(highPriorLowNoiseJS(highPriorLowNoiseRange), highPriorLowNoiseResp(highPriorLowNoiseRange), jmpspace);   
    ...
    [min(participant, 6), slope(participant, 6), threshold(participant, 6), max(participant, 6), lowPriorLowNoisePsychCurves(participant, :), ~] = psychCurve_4P(lowPriorLowNoiseJS(lowPriorLowNoiseRange), lowPriorLowNoiseResp(lowPriorLowNoiseRange), jmpspace);   
...
%     highPriorHighNoiseRange = 1 : floor(highPriorHighNoiseSize/2); %for first half
%     highPriorHighNoiseRange = (floor(highPriorHighNoiseSize/2) + 1): highPriorHighNoiseSize; %for second half    [coefs, dev, stats, highPriorHighNoisePsychCurves(participant, :), thresh50(participant, 7)] = psychCurves(highPriorHighNoiseJS(highPriorHighNoiseRange), highPriorHighNoiseResp(highPriorHighNoiseRange), 0.5, 4);
    [coefs, dev, stats, highPriorHighNoisePsychCurves(participant, :), thresh50(participant, 7)] = psychCurves(highPriorHighNoiseJS(highPriorHighNoiseRange), highPriorHighNoiseResp(highPriorHighNoiseRange), 0.5, 4);
    coefInt(participant, 7) = coefs(1);
    coefSlope(participant, 7) = coefs(2);
...
thresh50 = array2table(thresh50);
thresh50.Properties.VariableNames = names;
...
coefInt = array2table(coefInt);
coefInt.Properties.VariableNames = names;

coefSlope = array2table(coefSlope);
coefSlope.Properties.VariableNames = names;

save('summary_allTrials2.mat', 'baselinePsychCurves', 'lowPriorNoNoisePsychCurves', 'highPriorNoNoisePsychCurves', 'lowPriorLowNoisePsychCurves', 'highPriorLowNoisePsychCurves', 'lowPriorHighNoisePsychCurves', 'highPriorHighNoisePsychCurves', 'baselinePsychBins', 'lowPriorNoNoisePsychBins', 'highPriorNoNoisePsychBins', 'lowPriorLowNoisePsychBins', 'highPriorLowNoisePsychBins', 'lowPriorHighNoisePsychBins', 'highPriorHighNoisePsychBins', 'thresh50', 'CriterionVals', 'dPrimeVals', 'coefInt', 'coefSlope', 'binSize');
